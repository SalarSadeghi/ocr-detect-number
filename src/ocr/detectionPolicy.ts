...
export function evaluateAutomaticDetection({
  ...
  if (!consensus) {
    return {
      ...
      message:
        sharpness < 12
          ? "تصویر کمی تار است؛ دوربین را ثابت‌تر و به عدد نزدیک‌تر کنید."
          : highConfidence
            ? "عدد با اطمینان خوب خوانده شد؛ برای تأیید یک بار دیگر بررسی می‌شود."
            : "عدد با اطمینان پایین خوانده شد؛ برای تأیید دوباره بررسی می‌شود.",
    };
  }
